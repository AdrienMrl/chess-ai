...
# ---- Zobrist hashing tables (same seed/order as engine.py's zobrist_hash so
# incremental and full-rescan hashes are directly comparable) ----
_zrand = random.Random(0xC0FFEE)
ZOBRIST_PIECE = {c: [_zrand.getrandbits(64) for _ in range(64)] for c in "PNBRQKpnbrqk"}
ZOBRIST_SIDE = _zrand.getrandbits(64)
ZOBRIST_CASTLE = {c: _zrand.getrandbits(64) for c in "KQkq"}
ZOBRIST_EP_FILE = [_zrand.getrandbits(64) for _ in range(8)]
...
def zobrist_hash_full(board):
    """Full from-scratch recompute -- ground truth, used only for init/validation."""
    h = 0
    for s in range(64):
        c = board.squares[s]
        if c != '.':
...
# Precomputed square constants for hot paths (avoid repeated name_sq() calls).
...
class Board:
    ...
    def reset(self):
        ...
        self.uci_history = []
        self.king_sq = [E1, E8]  # cached king squares, indexed by color
        self.zobrist = zobrist_hash_full(self)  # incrementally maintained Zobrist key
    ...
    def load_fen(self, fen):
        ...
        self.history = []
        self.uci_history = []
        self.king_sq = [self.squares.index("K"), self.squares.index("k")]
        self.zobrist = zobrist_hash_full(self)  # full recompute on load (rare event)
    ...
    def _legal_moves_slow(self):
        """Original make+is_attacked+unmake legality verification for every
        pseudo-legal move. Used as a fallback when the side to move is in
        check (evasions are rare and trickier to fast-path safely)."""
        legal = []
        color = self.turn
    ...
    def _pinned_pieces(self, ks, color):
        """Compute pieces of `color` pinned to the king on square `ks`.

        Returns a dict: pinned_square -> (dx, dy) direction of the pin ray
        (the line through the king and the pinned piece). A move of that
        piece is legal only if its destination stays collinear with the
        king along this line.
        """
        pinned = {}
        f0, r0 = file_of(ks), rank_of(ks)
        for df, dr in BISHOP_DIRS:
            f, r = f0 + df, r0 + dr
            first = None
            while 0 <= f < 8 and 0 <= r < 8:
                ...
                if c != ".":
                    if first is None:
                        if self.color_of(c) == color:
                            first = sq(f, r)
                        else:
                            break
                    else:
                        if self.color_of(c) != color and c.upper() in ("B", "Q"):
                            pinned[first] = (df, dr)
                        break
                f += df
                r += dr
            ...
            while 0 <= f < 8 and 0 <= r < 8:
                ...
                if c != ".":
                    if first is None:
                        ...
                        if self.color_of(c) != color and c.upper() in ("R", "Q"):
                            pinned[first] = (df, dr)
                        break
                f += df
                r += dr
    ...
    def legal_moves(self):
        ...
        # If the king is missing (shouldn't happen in real games/perft
        # positions) or currently in check, fall back to the slow, always
        # correct path -- check evasions are rare and easy to get wrong
        # with fast-path shortcuts.
        if ks is None or self.is_attacked(ks, enemy):
            return self._legal_moves_slow()

        pinned = self._pinned_pieces(ks, color)
        f0, r0 = file_of(ks), rank_of(ks)
        ...
        for m in self.pseudo_legal_moves():
            ...
            if m.flag == "ep":
                # En passant can rarely expose a discovered check along the
                # king's rank when both pawns disappear simultaneously.
                # Not worth special-casing: verify with make/unmake.
                undo = self._make(m)
                if not self.is_attacked(self.king_square(color), enemy):
                    legal.append(m)
                self._unmake(undo)
                continue
            ...
            if p == "K":
                if m.flag in ("ck", "cq"):
                    # Castling legality (through-check squares) is already
                    # verified in _castle_moves via is_attacked.
                    legal.append(m)
                    continue
                # Temporarily vacate the king's origin square: the king may
                # currently be blocking a sliding attack through itself, so
                # is_attacked(dest) must not see the king as a blocker.
                self.squares[m.frm] = "."
                attacked = self.is_attacked(m.to, enemy)
            ...
            if m.frm in pinned:
                dx, dy = pinned[m.frm]
                tf, tr = file_of(m.to), rank_of(m.to)
                # Legal only if destination stays on the line through the
                # king in direction (dx, dy) -- collinearity via cross
                # product against the king's square.
                if (tf - f0) * dy == (tr - r0) * dx:
                    legal.append(m)
                continue

            # Not pinned, not the king, not en passant: moving this piece
            # cannot expose the king to check while the king isn't already
            # in check, so the pseudo-legal move is automatically legal.
            legal.append(m)
        return legal
    ...
    def _make(self, m):
        ...
        h = self.zobrist
        h ^= ZOBRIST_PIECE[piece][m.frm]  # lift moving piece off its origin

        self.squares[m.frm] = "."
        # place piece (with promotion)
        if m.promo:
            new_piece = m.promo.upper() if color == WHITE else m.promo.lower()
        else:
            new_piece = piece
        if captured != ".":
            h ^= ZOBRIST_PIECE[captured][m.to]  # remove normally-captured piece
        self.squares[m.to] = new_piece
        h ^= ZOBRIST_PIECE[new_piece][m.to]  # place moved (possibly promoted) piece

        if piece == "K" or piece == "k":
            self.king_sq[color] = m.to

        # en passant capture removes the pawn behind
        if m.flag == "ep":
            cap_sq = m.to + (-8 if color == WHITE else 8)
            ep_captured = self.squares[cap_sq]
            h ^= ZOBRIST_PIECE[ep_captured][cap_sq]
            self.squares[cap_sq] = "."
        ...
        if m.flag == "ck":
            if color == WHITE:
                h ^= ZOBRIST_PIECE["R"][H1]
                h ^= ZOBRIST_PIECE["R"][F1]
                self.squares[F1] = "R"
                self.squares[H1] = "."
            else:
                h ^= ZOBRIST_PIECE["r"][H8]
                h ^= ZOBRIST_PIECE["r"][F8]
                self.squares[F8] = "r"
                self.squares[H8] = "."
        elif m.flag == "cq":
            if color == WHITE:
                h ^= ZOBRIST_PIECE["R"][A1]
                h ^= ZOBRIST_PIECE["R"][D1]
                self.squares[D1] = "R"
                self.squares[A1] = "."
            else:
                h ^= ZOBRIST_PIECE["r"][A8]
                h ^= ZOBRIST_PIECE["r"][D8]
                self.squares[D8] = "r"
                self.squares[A8] = "."
        ...
        for right in "KQkq":
            if (right in old_castling) != (right in self.castling):
        ...
        if old_ep is not None:
            h ^= ZOBRIST_EP_FILE[file_of(old_ep)]
        if self.ep is not None:
            h ^= ZOBRIST_EP_FILE[file_of(self.ep)]
        ...
        if color == BLACK:
            self.fullmove += 1
        self.turn = 1 - color
        h ^= ZOBRIST_SIDE  # side-to-move always flips
        self.zobrist = h
        return undo
